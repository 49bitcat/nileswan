...
static void mcu_spi_configure_dma_tx_data(const void *address, uint32_t length) {
    LL_DMA_SetMemoryAddress(DMA1, MCU_DMA_CHANNEL_SPI_TX_DATA, (uint32_t) address);
...
void mcu_spi_init(mcu_spi_mode_t mode) {
    ...
    if (dma_enabled) {
        ...
        LL_DMA_SetPeriphAddress(DMA1, MCU_DMA_CHANNEL_SPI_TX_DATA, LL_SPI_DMA_GetRegAddr(MCU_PERIPH_SPI));
    }
